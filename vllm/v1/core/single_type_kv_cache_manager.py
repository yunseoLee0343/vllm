...
class MambaManager(SingleTypeKVCacheManager):
    def __init__(
        ...
        if self.mamba_cache_mode == "align":
            ...
            # Requests currently advancing through a sub-block tail where
            # persistence can be deferred.
            self._deferred_tail_reqs: set[str] = set()

    def _has_uncached_tail(self, num_tokens_main_model: int) -> bool:
        return num_tokens_main_model % self.block_size != 0
    ...
    def get_num_blocks_to_allocate(
        ...
        if (
            ...
            # For a sub-block tail, allow compute progress without requiring
            # immediate persistent block allocation in this step.
            if (
        ...
        if self.mamba_cache_mode != "align":
            ...
            if (
                num_new_blocks > 0
                and self._should_defer_tail_persistence(
                    request_id, num_tokens_main_model
                )
            ):
                # Defer persistence for sub-block tails to avoid unnecessary
                # allocation churn near block boundaries.
                num_new_blocks = 0
