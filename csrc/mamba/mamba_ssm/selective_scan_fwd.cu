...
void selective_scan_fwd_kernel(SSMParamsBase params) {
    ...
    const auto u_ptr = params.u_ptr;
    const auto delta_ptr = params.delta_ptr;
    const auto A_ptr = params.A_ptr;
    const auto B_ptr = params.B_ptr;
    const auto C_ptr = params.C_ptr;
    const auto ssm_states_ptr = params.ssm_states_ptr;
    ...
    input_t *u = reinterpret_cast<input_t *>(u_ptr) + sequence_start_index * u_batch_stride
        + dim_id * kNRows * u_d_stride;
    input_t *delta = reinterpret_cast<input_t *>(delta_ptr) + sequence_start_index * delta_batch_stride
        + dim_id * kNRows * delta_d_stride;
    weight_t *A = reinterpret_cast<weight_t *>(A_ptr) + dim_id * kNRows * params.A_d_stride;
    weight_t *B = reinterpret_cast<weight_t *>(B_ptr) + dim_id * kNRows * params.B_d_stride;
    input_t *Bvar = reinterpret_cast<input_t *>(B_ptr) + sequence_start_index * params.B_batch_stride + group_id * params.B_group_stride;
    weight_t *C = reinterpret_cast<weight_t *>(C_ptr) + dim_id * kNRows * params.C_d_stride;
    input_t *Cvar = reinterpret_cast<input_t *>(C_ptr) + sequence_start_index * params.C_batch_stride + group_id * params.C_group_stride;

    typename Ktraits::state_t *ssm_states;
    if (params.cache_enabled) {
        // APC mode: ssm_states points to the base, we'll use absolute cache slots later
        ssm_states = reinterpret_cast<typename Ktraits::state_t *>(ssm_states_ptr) +
