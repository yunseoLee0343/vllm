...
def run(args: argparse.Namespace) -> dict[str, float | int]:
    ...
    return {
        ...
        "tokens_per_sec": eff_tokens_per_sec,
        "stall_ratio": stall_ratio,
        "scheduler_efficiency": scheduler_efficiency,
        "useful_compute_ratio": useful_compute_ratio,
        "compute_efficiency": compute_efficiency,
        "state_write_bytes": state_write_bytes,
        "bytes_per_token": bytes_per_token,
        "memory_bound_indicator": memory_bound_indicator,
        "memory_efficiency": memory_efficiency,
        "zero_fill_time_fraction": zero_fill_fraction,
...
CSV_PATH = Path("mamba_roofline.csv")
CSV_COLUMNS = [
    "experiment_id",
    "seqlen",
    "dim",
    ...
    "state_write_bytes",
    "bytes_per_token",
    "memory_bound_indicator",
    "memory_efficiency",
    "zero_fill_time_fraction",
    "kernel_calls",
    "total_chunks",
    "avg_chunk_tokens",
]
...
def append_roofline_csv(experiment_id: str, args: argparse.Namespace,
    ...
    with CSV_PATH.open("a", newline="", encoding="utf-8") as f:
        ...
        writer.writerow({
            ...
            "stall_ratio": metrics["stall_ratio"],
            "scheduler_efficiency": metrics["scheduler_efficiency"],
            "useful_compute_ratio": metrics["useful_compute_ratio"],
            "compute_efficiency": metrics["compute_efficiency"],
            "state_write_bytes": metrics["state_write_bytes"],
            "bytes_per_token": metrics["bytes_per_token"],
            "memory_bound_indicator": metrics["memory_bound_indicator"],
            "memory_efficiency": metrics["memory_efficiency"],
            "zero_fill_time_fraction": metrics["zero_fill_time_fraction"],
            "kernel_calls": metrics["kernel_calls"],
            "total_chunks": metrics["total_chunks"],
            "avg_chunk_tokens": metrics["avg_chunk_tokens"],
        })


def _assert_no_nan(row: dict[str, str]) -> None:
    for key, value in row.items():
        if key == "experiment_id":
            continue
        numeric = float(value)
        if math.isnan(numeric):
            raise ValueError(f"NaN detected in CSV column '{key}'")


def run_smoke_test_suite(base_args: argparse.Namespace) -> None:
    configs: list[tuple[str, argparse.Namespace]] = []

    default_args = deepcopy(base_args)
    default_args.force_aligned = False
    default_args.force_unaligned = False
    configs.append(("default", default_args))

    aligned_args = deepcopy(base_args)
    aligned_args.force_aligned = True
    aligned_args.force_unaligned = False
    configs.append(("force_aligned", aligned_args))

    unaligned_args = deepcopy(base_args)
    unaligned_args.force_aligned = False
    unaligned_args.force_unaligned = True
    configs.append(("force_unaligned", unaligned_args))

    run_id_prefix = f"seq{base_args.seqlen}_dim{base_args.dim}_dstate{base_args.dstate}"
    for name, cfg in configs:
        metrics = run(cfg)
        append_roofline_csv(f"{run_id_prefix}_{name}", cfg, metrics)

    if not CSV_PATH.exists():
        raise FileNotFoundError(f"Expected CSV output file missing: {CSV_PATH}")

    with CSV_PATH.open("r", newline="", encoding="utf-8") as f:
        rows = list(csv.DictReader(f))
    if len(rows) < 3:
        raise ValueError(f"Expected at least 3 CSV rows, found {len(rows)}")
    for row in rows[-3:]:
        _assert_no_nan(row)
